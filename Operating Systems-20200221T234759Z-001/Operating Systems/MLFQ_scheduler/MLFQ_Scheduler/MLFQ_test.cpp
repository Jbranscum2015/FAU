...
public:
	//constructors
	Process(string, vector<int>, int, vector<int>, int, int, int, int, int, int, int);
...
private:
	...
	int newQuantum; //mlfq time quantum
	int newLevel; //keep track of priority lvl of the process: 1 = RR TQ 4, 2 = RR TQ 9, 3 = FCFS
...
Process::Process(string processName, vector<int> cpuBurst, int CpuBurstIndex, vector<int> ioTime, int IoIndex, int waitTime, int responseTime,
	int turnAroundTime, int state, int quantum, int level) {
	newProcessName = processName;
...
void LoadtoReadyQueue(Process * process, deque<Process*>& readyQueue);
...
void PrintOutput(ofstream& output, deque<Process*> readyQueue, vector<Process*> IOlist, 
	vector<Process>& processes, int runningProcessIndex, vector<string>& completed, int timeUnit, int done);
void PrintReadyQueue(ofstream& output, deque<Process*> readyQueue);
...
int main() {
	...
	deque<Process*> queue1, queue2, queue3; //3 queues to represent the queues for each priority level
	deque<Process*> queue[3] = { queue1, queue2, queue3 };
	deque<Process*> readyQueue; //ready queue will be a queue that contains the addresses of the processes waiting for CPU time
		...
		if (runningProcessindex >= 0) { //only runs if there's a running process
			...
			if ((processes[runningProcessindex].getCpuBurst())[0] <= 0) {
				...
				if (readyQueue.size() > 0) {// if the ready queue has another process, then set up the process to be the running process in the next unit of time
					...
					readyQueue.pop_front(); //removes that process from the ready queue
				}
...
void InitializeProcesses(vector<Process>& processes) {
	...
	Process p1 = { processName, cpuBurst, CpuBurstIndex, ioTime, IoIndex, waitTime, responseTime,
		turnAroundTime, state, quantum, level};
...
void PrintOutput(ofstream& output, deque<Process*> readyQueue, vector<Process*> IOlist, vector<Process>& processes, int runningProcessIndex, vector<string>& completed, int timeUnit, int done) {
	cout << "Current Time: " << timeUnit << endl << endl;

	if (readyQueue.size() <= 0 && runningProcessIndex < 0) {
		//for debug purposes
		//output << "Current Process on the CPU: [idle]" << endl;
		cout << "The Next Process On the CPU: [idle]" << endl;
	}
		...
		//for debug purposes
		cout << "Current Process on the CPU: " << processes[runningProcessIndex].getProcessName() << endl;
...
void PrintReadyQueue(ofstream& output, deque<Process*> readyQueue) {
	...
	if (count > 0) {
		for (int i = 0; i < count; i++) {
			cout << setw(5) << (*(readyQueue.front())).getProcessName() << setw(10) <<
...
void PrintIOlist(ofstream& output, vector<Process*> IOlist) {
	...
	if (count > 0) {
		for (int i = 0; i < count; i++) {
			output << setw(5) << (*IOlist[i]).getProcessName() << setw(10) << ((*IOlist[i]).getIoTime())[0] << endl << endl;
		}
	}
